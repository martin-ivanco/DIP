...
void Glimpses::composeView(int phi, int lambda) {
    ...
    for (int i = 0; i < this->splits.size(); i++) {
        clip.open(this->splits[i].path);
        if (! clip.isOpened())
            throw runtime_error("Could not open split!");
        
        string glimpsePath = fs::path(this->folderPath) / this->getGlimpseName(i, phi, lambda);
        VideoInfo glimpse = VideoInfo(glimpsePath, static_cast<double>(this->originalVideo.fps), cv::Size(GLIMPSE_WIDTH, GLIMPSE_HEIGHT), i, phi, lambda);
    ...
    for (auto p : PHIS) {
        for (auto l : LAMBDAS) {
            for (int i = 0; i < this->splits.size(); i++) { 
                ...
                VideoInfo glimpse = VideoInfo(glimpsePath, static_cast<double>(this->originalVideo.fps), cv::Size(GLIMPSE_WIDTH, GLIMPSE_HEIGHT), i, p, l);
                this->glimpses.push_back(glimpse);
            }
        }
    }
